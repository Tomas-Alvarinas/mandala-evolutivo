...
import { deleteSolarReturnAction } from "@/lib/solar-returns/actions";
import { formatSolarReturnPeriod } from "@/lib/solar-returns";

type DeleteSolarReturnControlProps = {
  clientId: string;
...
export function DeleteSolarReturnControl({
  clientId,
  solarReturnId,
  periodStart,
  periodEnd,
}: DeleteSolarReturnControlProps) {
  ...
  const periodLabel = formatSolarReturnPeriod({ periodStart, periodEnd });
  ...
  async function handleDelete() {
    ...
    const result = await deleteSolarReturnAction({
      clientId,
      solarReturnId,
    });
    ...
    router.push(`/clients/${clientId}/solar-returns`);
    router.refresh();
  }
          ...
          >
            ...
            <div id={descriptionId} className="mt-4 space-y-3">
              <p className="text-sm leading-relaxed text-foreground">
                Se eliminarán los datos cargados y los informes de esta
                Revolución Solar del {periodLabel}.
              </p>
