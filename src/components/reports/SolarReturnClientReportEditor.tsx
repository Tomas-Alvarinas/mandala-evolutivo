...
import { updateSolarReturnClientReport } from "@/lib/reports/solar-returns/client-report/actions";
import {
...
type SolarReturnClientReportEditorProps = {
  clientId: string;
...
export function SolarReturnClientReportEditor({
  clientId,
  solarReturnId,
  reportId,
  report,
}: SolarReturnClientReportEditorProps) {
  const router = useRouter();
  const viewHref = `/clients/${clientId}/solar-returns/${solarReturnId}/reports/${reportId}/client`;
  const [draft, setDraft] = useState<SolarReturnClientReport>(report);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await updateSolarReturnClientReport({
      clientId,
      solarReturnId,
