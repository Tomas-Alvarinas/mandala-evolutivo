...
import { updateTransitAnalysisProfessionalReport } from "@/lib/reports/transits/actions";
import {
...
type TransitAnalysisReportEditorProps = {
  clientId: string;
...
export function TransitAnalysisReportEditor({
  clientId,
  transitAnalysisId,
  reportId,
  report,
}: TransitAnalysisReportEditorProps) {
  const router = useRouter();
  const detailHref = `/clients/${clientId}/transits/${transitAnalysisId}/reports/${reportId}`;
  const [draft, setDraft] = useState<TransitAnalysisReport>(report);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await updateTransitAnalysisProfessionalReport({
      clientId,
      transitAnalysisId,
      reportId,
      report: draft,
